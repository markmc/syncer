...
import (
	...
	"github.com/kcp-dev/syncer/pkg/cliplugins/workload/plugin"
)
...
func New(streams genericclioptions.IOStreams) (*cobra.Command, error) {
	...
	enableSyncerCmd := &cobra.Command{
		...
		Example:      fmt.Sprintf(syncExample, "kubectl syncer"),
		SilenceUsage: true,
	...
	cordonCmd := &cobra.Command{
		...
		Example:      fmt.Sprintf(cordonExample, "kubectl syncer"),
		SilenceUsage: true,
		RunE: func(c *cobra.Command, args []string) error {
			if len(args) != 1 {
				return c.Help()
	...
	uncordonCmd := &cobra.Command{
		...
		Example:      fmt.Sprintf(uncordonExample, "kubectl syncer"),
		SilenceUsage: true,
		RunE: func(c *cobra.Command, args []string) error {
			if len(args) != 1 {
				return c.Help()
	...
	drainCmd := &cobra.Command{
		...
		Example:      fmt.Sprintf(drainExample, "kubectl syncer"),
		SilenceUsage: true,
		RunE: func(c *cobra.Command, args []string) error {
			if len(args) != 1 {
				return c.Help()
