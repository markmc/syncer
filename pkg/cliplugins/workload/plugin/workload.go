...
func (o *CordonOptions) Run(ctx context.Context) error {
	...
	syncerClient, err := syncerclient.NewForConfig(config)
	if err != nil {
		return fmt.Errorf("failed to create kcp client: %w", err)
	}

	syncTarget, err := syncerClient.WorkloadV1alpha1().SyncTargets().Get(ctx, o.SyncTarget, metav1.GetOptions{})
	if err != nil {
	...
	_, err = syncerClient.WorkloadV1alpha1().SyncTargets().Patch(ctx, o.SyncTarget, types.JSONPatchType, patchBytes, metav1.PatchOptions{})
	if err != nil {
